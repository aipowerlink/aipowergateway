...
### D6.5.1：内置角色只读、复制后定制（客户随时切换，内置不可改）【补充：用户提问】

**问题**：客户可随时切换到内置角色，但内置角色（server/client）是否允许修改？

**决策：内置角色（system trust）只读不可改；定制 = 复制为自定义角色后修改**——参考 DSH preset 信任模型（trust 来自根目录而非文件，shipped root 的预设天然只读）

1. **内置角色（system trust）**：
   - 出厂预设，随安装包分发（只读，不可 edit/rm）
   - 升级随版本覆盖——保证可预期性（升级后仍回到标准 server/client 行为）
2. **定制路径（唯一方式）**：`role clone server my-server-custom` → 在用户数据目录生成副本（user trust）→ 编辑副本模块集
   - 复制不破坏内置：内置始终可回退（客户随时 `--role server` 切回）
3. **信任不可伪装**（DSH 同款）：trust 由所在目录决定（内置目录=system，用户目录=user），角色文件内不可声明——本地角色不能伪装成内置
4. **切换语义**：
   - `--role server` / `--role client` / `--role <custom-id>` 随时切换
   - 自定义角色与内置并存；`role list` 显示全部（内置带 system 标记，自定义带 user 标记）
5. **升级兼容**：内置角色结构变更时，旧自定义角色（user）保留原样；如继承基底（default: server）则重新合并
6. **CLI 约束**：`role edit server` → 拒绝（提示只读，建议 clone）；`role rm server` → 拒绝

**对客户的价值**：既保证"随时回退到标准行为"（内置只读），又满足"独特定制"（复制即改）；内置与自定义互不污染。
