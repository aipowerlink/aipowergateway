...
- [ ] 4.1 实现 lan-discovery-client：UDP 监听广播 + 主动扫描，组长列表（去重刷新/离线移除），验证发现与去重
- [ ] 4.2 实现 lan-share-client：选择组长 + 密码 → 换 Bearer token，验证接入成功/失败
- [ ] 4.3 实现 OpenAI 兼容调用（带 token），验证标准响应接收
- [ ] 4.4 实现 Anthropic 兼容调用（含 SSE 流式消费），验证流式响应接收
- [ ] 4.5 实现被踢/改密即时失效（401 拒绝 + 明确提示），验证失效提示
- [ ] 4.6 实现 lan-identity：换 token 上报机器名/显示名（默认=机器名）+ 改名同步，验证上报与改名
- [ ] 4.7 实现 lan-usage-view：响应 usage 累计 + 个人用量查看，验证累计与展示
- [ ] 4.8 实现组长离线不卡死（请求超时 → 标记离线 + 保留配置），验证超时行为
- [ ] 4.9 实现组长列表持久化（client_config 表），验证重启后列表保留
