...
- **自动获取具体模型列表**（参考 cc-switch「获取模型」）：表单「获取模型」按钮按当前填写的内容探测端点（OpenAI 兼容 `GET {base_url}/models` → `data[].id`，自动去重），把模型服务器返回的**真实模型清单**填入模型 chips；保存提供方时若未显式配置模型（models 为空），网关自动拉取该提供方的真实模型列表并落盘，`/v1/models` 即刻生效——显式配置的模型列表不会被覆盖。
