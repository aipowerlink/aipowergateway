...
//! 请求经 UDP 发现的组长共享通道端口转发给组长 gateway（gateway 间通信）。
...
impl MemberGateway {
    ...
    /// 选择组长：当前在线的组长中最近发现的（last_seen 最新）。
    ...
    pub async fn proxy(
        ...
        let leader = self.pick_leader().ok_or_else(|| "no leader discovered yet (check LAN + leader sharing)".to_string())?;
